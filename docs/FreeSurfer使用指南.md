...
TwinBrain 支持使用 FreeSurfer 表面文件来**创建 Unity 前端的基础结构**。这是一次性的设置步骤，之后您可以加载各种数据源进行可视化。
...
#### 步骤 1：创建前端结构（使用 FreeSurfer）
FreeSurfer 文件定义大脑的**空间结构**：
- 🧠 脑区的3D位置
- 🏷️ 脑区标签和名称
- 🌐 网络归属
- 🎨 默认颜色
...
✅ **FreeSurfer = 前端结构**（一次性创建）
✅ **数据 = 内容**（可以随时更换）
✅ **不需要两个数据源同时存在**
❌ **不是** FreeSurfer + 数据必须一起使用
